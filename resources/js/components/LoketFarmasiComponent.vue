                  ...
                  <div class="col">
                    <div>
                      <a class="text-inherit"
                        >Nomor Antri <b>{{ antri.tiket_farmasi_nomor }}</b></a
                      >
                    </div>
                  ...
                  <div class="col">
                    <div>
                      ...
                      >
                        <i
                          ...
                          v-show="antri.tiket_farmasi_status == 1"
                        ></i>
                        <i
                          class="fa fa-minus-circle text-danger"
                          v-show="antri.tiket_farmasi_status == 2"
                        ></i>
                        Nomor Antri <b>{{ antri.tiket_farmasi_nomor }}</b>
                      </a>
                    </div>
                    ...
                    <div class="dropdown pull-right" style="margin-top:-20px">
                      ...
                      >
                        <a
                          class="dropdown-item"
                          v-if="antri.tiket_farmasi_status == 1"
                          href="javascript:void(0)"
                    ...
                    <small
                      ...
                      v-if="antri.tiket_farmasi_status == 1"
                      ><b>Jam Dilayani :</b>
                      {{ antri.tiket_farmasi_accepted }}</small
                    >
                    <small
                      class="d-block item-except text-sm text-muted h-1x"
                      v-else
...
export default {
    ...
    cari: function(newVal, oldVal) {
      ...
      if (newVal != "" && this.completed.length > 0) {
        this.antri_completed = this.completed.filter(i =>
          i.tiket_farmasi_nomor.toLowerCase().includes(newVal.toLowerCase())
        );
      } else {
  ...
  computed: {
    show_success_attend() {
      return this.antri_current.tiket_farmasi_acceptor != null;
    },
    ...
    show_attend() {
      ...
      return this.antri_current.tiket_farmasi_nomor != "-";
    },
    ...
    disabled_selesai() {
      return (
        ...
        this.antri_current.tiket_farmasi_nomor == "-" ||
  ...
  methods: {
    save_config() {
      let self = this;
      axios.post("farmasi/save", { farmasi: this.farmasi }).then(response => {
    ...
    parse_antri(data) {
      ...
      for (let x in data) {
        ...
        if (data[x].tiket_farmasi_status == 0) {
          if (current) {
        ...
        } else if (data[x].tiket_farmasi_status > 0) {
    ...
    init_data() {
      ...
      axios
        .post("farmasi/antrian")
        .then(response => vm.parse_antri(response.data));

      axios
        .post("farmasi/get/" + this.id_poli)
        .then(response => (this.farmasi = response.data));

      axios.post("farmasi/summary-rujuk").then(response => {
    ...
    panggil_ulang() {
      let nomor = this.antri_current.tiket_farmasi_nomor;
      ...
      axios
        ...
        .then(response => {
          this.call_number(this.antri_current.tiket_farmasi_nomor);
    ...
    panggil_selanjutnya() {
      let next = this.antri_waiting[0].tiket_farmasi_nomor;

      if (this.antri_current.tiket_farmasi_acceptor == null) {
        let nomor = this.antri_current.tiket_farmasi_nomor;
        ...
        Swal.fire({
          ...
          if (result.value) {
            this.set_disabled_button(true);
            axios
              .post("farmasi/next", { farmasi: this.antri_current, next: next })
              .then(
                response => (
        ...
        this.set_disabled_button(true);
        axios.post("farmasi/next", { farmasi: this.antri_current, next: next }).then(
          response => (
            this.call_number(next),
            this.init_data(),
            this.ws.send(
    ...
    selesai() {
      if (this.antri_current.tiket_farmasi_acceptor == null) {
        ...
        Swal.fire({
          ...
          if (result.value) {
            axios.post("farmasi/end", { farmasi: this.antri_current }).then(
              response => (
                this.init_data(),
                this.ws.send(
                  JSON.stringify({
    ...
    set_attend(farmasi, value) {
      let self = this;
      axios.post("farmasi/attend", { farmasi, value }).then(response => {
        if (!value) {
    ...
    set_rujuk(farmasi, value) {
      if (!value && farmasi.tiket_pasien_dirujuk == "0") return false;
      if (value && farmasi.tiket_pasien_dirujuk == "1") return false;

      let self = this;
      axios.post("farmasi/rujuk", { farmasi, value }).then(response => {
        if (value) {
          if (farmasi.tiket_farmasi_status != 1) {
            self.set_attend(farmasi, true);
          }
        }
      ...
      this.ws.onmessage = function(e) {
        ...
        if (data.target == "loket") {
          ...
          if (data.sub_target == "farmasi") {
            if (data.gedung == self.farmasi.poli_gedung) {
